...
<!-- <?= DetailView::widget([
        ...
        'attributes' => [
            ...
            [
                ...
                'format' => 'image',
                'value'=>function($model) { return $model->employerlogo_base_url.'/'.$model->employerlogo_path; },
            ...
            [   
                ...
                'value' => ($model->districts!='')? $model->districts->district:'',
...
<?php
    ...
    if(isset($model->link)) echo Html::a("More details, click here", Url::to($model->link), ["target" => "_blank", "class" => "btn btn-info"]).'&nbsp;';
?>
<?php if(Yii::$app->user->can('user')) { ?>
    <?php if($apply->applied($model->id) == 0 && $model->apply_through_kora_flag==1) { ?>
    <?php 
        Modal::begin([
            'header' => 'Apply Now:'.$model->jobtitle,
            "class" => "vd_bg-red", 
            'toggleButton' => [
        ...
        Modal::end();
    } else if ($apply->applied($model->id) != 0 && $model->apply_through_kora_flag == 1) {
        echo "<span class='btn btn-large btn-default'>Applied </span>";
    }
}
echo "&nbsp;<span class='btn btn-large btn-info'> Viewed: ".Yii::$app->jobSeeker->numberViewedApportunity($model->id)."</span>";
?>
