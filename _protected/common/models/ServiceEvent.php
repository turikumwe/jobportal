...
class ServiceEvent extends \yii\db\ActiveRecord {
    ...
    public function rules() {
        return [
            [['event_title', 'description_event', 'start_date', 's_opportunity_id', 'apply_through_kora_flag'], 'required'],
//            ['closure_date', 'required', 'when' => function ($model) {
//                    return $model->always_open_flag == '0';
//                }],
            [
                ['closure_date'], 'required', 'when' => function ($model) {
                    return $model->always_open_flag == 0;
                }, 'whenClient' => "function (attribute, value) {
                    return $('#serviceevent-always_open_flag').val() == 0;
                }", 'enableClientValidation' => true
            ],
            [['posted', 'action_id', 'created_by', 'deleted_by', 'updated_by', 'date_posted', 's_opportunity_id', 'number_participant', 'event_duration', 'apply_through_kora_flag','always_open_flag'], 'integer'],
            [['doc_path'], 'string', 'max' => 255],
            [['event_summary', 'event_requirement', 'doc_path', 'how_to_apply', 'description_organiser', 'description_event', 'qualification_participant'], 'string'],
            [['start_date', 'closure_date', 'created_at', 'deleted_at', 'updated_at', 'end_date'], 'safe'],
    ...
    public function attributeLabels() {
        return [
            ...
            'docFile' => Yii::t('frontend', 'Attach document'),
            ...
            'always_open_flag' => Yii::t('frontend', 'No application deadline'),
